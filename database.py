...
def add_to_favorites(user_id: int, listing: dict):
    conn = sqlite3.connect(DB_PATH)
    cursor = conn.cursor()
    try:
        cursor.execute("""
            INSERT INTO favorites (user_id, listing_id, title, price, url)
            VALUES (?, ?, ?, ?, ?)
        """, (user_id, listing["id"], listing["title"], listing["price"], listing["url"]))
        conn.commit()
    except sqlite3.IntegrityError:
        pass
    conn.close()
...
def get_favorites(user_id: int) -> list:
    ...
    cursor = conn.cursor()
    cursor.execute("""
        SELECT listing_id, title, price, url, added_at 
        FROM favorites 
        WHERE user_id = ? 
...
def remove_from_favorites(user_id: int, listing_id: str):
    conn = sqlite3.connect(DB_PATH)
    cursor = conn.cursor()
    cursor.execute("DELETE FROM favorites WHERE user_id = ? AND listing_id = ?", (user_id, listing_id))
    conn.commit()
    conn.close()
