...
async def set_date(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [
        ...
        [InlineKeyboardButton("🔄 Начать заново", callback_data="set_filters")],
        [InlineKeyboardButton("❌ Отмена", callback_data="cancel")],
    ]
...
async def cancel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("❌ Настройка отменена.", reply_markup=get_main_menu())
    return ConversationHandler.END
...
async def show_filters(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [
        [InlineKeyboardButton("🔧 Изменить", callback_data="set_filters")],
...
async def search_now(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await message("🔍 *Ищу квартиры...*", parse_mode="Markdown")
...
async def add_favorite(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("fav_"):
        ...
        # Получаем данные объявления из контекста или парсим заново
        # Для простоты — просто сообщаем, что добавлено
        await query.edit_message_reply_markup(
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton("🔗 Открыть на Krisha", url=query.message.reply_markup.inline_keyboard[0][0].url)],
                [InlineKeyboardButton("✅ В избранном", callback_data="done")]
            ])
        )
        await context.bot.send_message(
            chat_id=update.effective_chat.id,
...
async def stop_notifications(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text += "Фильтры удалены. Чтобы возобновить, настрой фильтры заново."
    if query:
...
def main():
    ...
    application.add_handler(CallbackQueryHandler(add_favorite, pattern="^fav_"))
